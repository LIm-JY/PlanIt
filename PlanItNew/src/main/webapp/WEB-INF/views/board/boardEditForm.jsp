	...
	<div class="boardListcontainer"
		...
		<div class="card-title" id="card-title" style="text-align: center;">
			...
			<h3 id="loginFormLogo" class="card-title text-center">community
				Edit</h3>
			<br> <br>
			<p id="ptext" align="left">${loginInfo.uname}님이수정중입니다.</p>
			<br>
			<hr>
		</div>
		...
		<div class="card-body" id="card-body" style="text-align: center;">
			...
			<form method="post" enctype="multipart/form-data">
				...
				<table id="boardlistTable">
					<tr>
						<td><input type="hidden" name="uidx" id="uidx"
							value="${board.uidx}"></td>
						<td><input type="hidden" name="bidx" value="${board.bidx}"
							readonly></td>
					</tr>
					<tr>
						<td><label for="btitle">제목</label></td>
						<td id="inputTd"><input type="text" name="btitle" id="btitle"
							minlength="4" maxlength="10" value="${board.btitle}" required></td>
					</tr>
				...
				<table>
					...
					<tr>
						<td><label>현재 사진</label></td>
						
						<td>
						<input type="hidden" name="oldFile1" value="${board.bphoto1}">
						<c:if test="${not empty board.bphoto1}">
								<img src="${imagePath}/${board.bphoto1}" style="width: 80px;">
							</c:if> <c:if test="${empty board.bphoto1}">
							첨부된 이미지 없음
							</c:if></td>
						<td>
						<input type="hidden" name="oldFile2" value="${board.bphoto2}">
						<c:if test="${not empty board.bphoto2}">
								<img src="${imagePath}/${board.bphoto2}" style="width: 80px;">
							</c:if> <c:if test="${empty board.bphoto2}">
						...
						<td id="file_box_td">
							...
							<div class="file_box">
								<label for="input_file1">사진첨부</label> <input type="file"
									id="input_file1" class="file_hidden1"> <input
									class="bphoto1" value="선택파일 없음" name="bphoto1"
					...
					<tr>
						<td id="file_box_td">
							<div class="file_box">
								<label for="input_file2">사진첨부</label> <input type="file"
									id="input_file2" class="file_hidden2"> <input
